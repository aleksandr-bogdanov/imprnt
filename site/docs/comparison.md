...
imprnt does now measure itself, on its own task. A small in-repo eval runs plain-language questions against the curated example vaults and checks where the answer note ranks. Over 39 questions across two vaults it tops the right note on the first hit about 87% of the time, and lands it in the top five (the cheap set the model reads) about 95% of the time. The corpus is small and the questions are hand-written, so read this as an early signal that BM25 over a tagged vault answers the questions asked, not a leaderboard entry. The harness and queries live in [`eval/`](https://github.com/aleksandr-bogdanov/imprnt/tree/main/eval) so you can read the labels and run it yourself.

> **What this does not say.** That 95% is not mempalace's 0.96 minus a point. Different task, different corpus, a tenth the questions. The number is honest about imprnt's task and silent about LongMemEval's. If a big-corpus leaderboard score is what you need before committing, pick a tool that publishes one.
