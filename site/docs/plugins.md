...
> **In one line.** The core is your notes plus three commands. Everything else is an add-on you turn on when you want it and delete when you do not. The core never even knows the add-ons exist.

The core is your notes plus `ingest`, `recall`, and `check`. Every other behavior is a plugin you add when you want it and remove with one command.
...
If adding a plugin forces you to touch the core, the plugin is wrong, not the core. This is what keeps imprnt from slowly growing to know about every add-on, which is exactly what bloats this kind of system over time.

A plugin depends on two things only: your `vault/` notes (and their header format) and its own folder. Not core internals, not another plugin, not another plugin's folder.
...
- **Behavior plugins** hand the assistant a fixed chunk of text you wire into its prompt. The vault never force-feeds the assistant. Remove is deleting the line. anti-slop and character are these. There is no referee for clashing behaviors, you own how they combine.
