...
Two kinds of "memory" usually get jumbled together: what you **know**, and rules for how the assistant **behaves**. Every AI memory system has to answer one question for each thing it holds: where does this live, and will the assistant see it? Most answer with several overlapping stores, so you can never quite say where a fact went or whether it is in context right now. imprnt keeps one store for knowledge and one strict rule about what loads on its own.
...
- **Knowledge** is what you know about your world: people, policies, decisions, numbers. You want to find it when it matters and keep it out of the way the rest of the time, so it should be **searchable** and fetched on demand.
...
These pull in opposite directions. Knowledge wants to be big and quiet, behavior tiny and loud. Put them in one bucket and the whole thing turns into a junk drawer.
...
imprnt usually runs under Claude Code, which ships a memory of its own, separate from the vault. The assistant writes a `MEMORY.md` about a project as it works. The first stretch of it auto-loads every session, stays on your machine, and lives outside `vault/`, so `recall` never sees it. Useful for what the assistant learns about a repo. It is also a second **always-on** store you cannot search.
