...
For people working on imprnt itself, not using the vault. One repo, several published packages, ships **Node**, built with Bun. imprnt is open source ([MIT](https://github.com/aleksandr-bogdanov/imprnt/blob/main/LICENSE)).
...
- **`imprnt-plugin-*`** (`packages/plugin-*/`) is the gallery, each plugin its own package. The naming convention matches the shape ESLint uses, so anyone can publish a plugin later without joining an npm org. The core never imports a plugin and never names one.
...
The gallery is generic and shippable. The shipped character is **Scribe**, a default you copy and make yours, and the shipped anti-slop is the universal core. Your private instance lives in `plugins/_personal/`, gitignored and never published. To personalize, copy a gallery plugin into `_personal/`, edit it, and wire the local file directly. `imprnt plugin list` skips `_personal/`, so your private cast never shows in the public listing.
