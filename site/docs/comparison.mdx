...
The camp narrowed one more of imprnt's claims in August, and it should be said plainly: the typed contract is no longer imprnt's alone. NanoClaw's memory format ([OKF](https://github.com/nanocoai/nanoclaw/blob/main/docs/memory.md)) declares a `type` per Markdown file - `person`, `project`, `decision` - plus tags and links between entities, with no database and no embedding store. That is the contract idea this site describes, shipped inside a 30k-star assistant. What remains imprnt-only in that pairing is the deterministic half: alias resolution that merges a renamed person into one note, an integrity pass where code, not the model, checks orphan links and untagged notes, and a ranker, so recall stays cheap past the point where an agent grepping and walking hand-kept indexes starts paying tokens for every lookup.
