...
A third shape appeared in 2026 and now holds the field's two biggest repos. The personal-agent runtimes, OpenClaw and Hermes Agent, keep memory as notes and derive a search index from them, and in both the author of the notes is the agent itself. Where they land on this page's questions depends on which half you look at, so both are measured on the table below.
...
One row points the other way, and it comes from the other camp. Letta, a memory company, published a LoCoMo run where a plain-files approach scored 74.0%, ahead of its own specialized memory tools and of mem0's best variant at 68.5%. Published against a vendor's own prior product is the rare direction that earns trust, and Letta is now rebuilding its flagship around Markdown files tracked in git. The retrieval half of that bet has a second big-name convert: Hermes Agent, the 236k-star personal agent from Nous Research, does its cross-session recall with SQLite full-text search, and nothing vector-shaped appears anywhere in its documented memory architecture.
...
The two biggest repos on the page are the personal-agent runtimes: OpenClaw at roughly 388k stars and Hermes Agent at roughly 237k. Both keep canonical memory as plain Markdown you own and can read, which is the camp's good half. The read path deserves a closer look. Hermes searches with SQLite full-text and documents nothing vector-shaped. OpenClaw's [own memory docs](https://docs.openclaw.ai/concepts/memory) state the default plainly: "OpenClaw uses OpenAI embeddings by default." On a stock install, every memory the agent writes is sent to OpenAI's API to be embedded into a local index, and semantic recall depends on that provider from then on. Local embedding exists, behind explicit configuration. If you picked a files-you-own tool for the privacy, that default is worth knowing before the first note is written.
