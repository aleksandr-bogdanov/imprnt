...
- Integrity: nothing in NanoClaw checks the memory, and the docs are explicit that this is by design: "The format is a convention, not a gate. A file with missing or malformed frontmatter still works as memory; the agent repairs metadata when it next touches the file." (docs/memory.md, accessed 2026-08-27). That sentence is the whole split, since the structure itself has converged: their contract is a convention the model repairs, imprnt's is an invariant code refuses to guess about. `imprnt check` rebuilds the index deterministically and flags orphan links, untagged notes, and folder/field disagreement, with code as the referee - and the type/tag vocabulary grows through a human's ratification where OKF's is "the agent's vocabulary, not a fixed list."
