...
**Status (checked 2026-08-27):** active, hyperactive cadence - two major versions shipped in the eight weeks since the 2026-07-02 check (v2.0.0 on 2026-07-10, v3.0.0 on 2026-08-09), 18 more releases on top, latest v3.0.8 on 2026-08-23, which is also the `"pushed_at"` date. Stars 805, up from 325 (+148%, the biggest relative mover in this folder). `"archived": false`. The major bumps are engine and packaging changes, and the architecture direction held (details in the update section below). Source: https://api.github.com/repos/CodeAbra/iai-personal-memory-engine and its /releases (accessed 2026-08-27). The 2026-07-02 status (v1.2.1, 325 stars) is kept in the record below.

**Latest release:** v3.0.8, 2026-08-23 | **Stars:** 805, forks 94 (GitHub API, 2026-08-27) | **License:** MIT | **Hosting:** self-host (fully local, no cloud)

## Update 2026-08-27: two majors, its own Rust engine, and a BM25 lane

Everything below this section is the June/July record, left as written. The two-major-version jump is real but it is engine replacement and packaging, a repositioning of the pitch, and one genuine retrieval change. It is the same product doing the same thing.

**v2.0.0 (2026-07-10) swapped the engine's internals for their own Rust code.** Release notes, verbatim: "New Rust storage and graph engine — replaces the previous Python/hnswlib backend. Existing stores migrate automatically on first run"; "Desktop app — a local, loopback-only dashboard to watch the memory graph live" (Tauri, macOS and Linux); "New memory operations — document study/teach, anticipatory foresight packs, and combined lexical + semantic search"; "Dropped the last GPL-adjacent dependencies; the project now ships its own storage and graph engines under MIT." Source: https://github.com/CodeAbra/iai-personal-memory-engine/releases/tag/v2.0.0 (accessed 2026-08-27).

**v3.0.0 (2026-08-09) added languages and hardened the capture path.** "Memory can speak thirteen more languages, when you ask it to" (`iai lang add` opts a store into cs de es fr hi id it ja pt ru th vi zh, embedder `multilingual-e5-small` at a pinned revision, English stays the default). "The deferred capture spool is encrypted" (AES-256-GCM under the store's existing key). Key rotation became recoverable. Capture now "extracts entity anchors from every stored turn — backticked identifiers, at-prefixed handles ... mid-sentence proper nouns including Cyrillic — and writes them as `entity:` tags." The nightly reflection "can ride a second subscription" (Codex or Gemini CLI instead of Claude). Source: https://github.com/CodeAbra/iai-personal-memory-engine/releases/tag/v3.0.0 (accessed 2026-08-27).

**Distribution flipped from source-only to packaged.** The June record's "There is no PyPI package" is dead: "From PyPI — a prebuilt wheel carries the engine, so no clone and no toolchains: `pip install iai-pme`." There is also a one-command bootstrap script and a Claude Code marketplace plugin: "`/plugin marketplace add CodeAbra/iai-personal-memory-engine`" then "`/plugin install iai-memory@iai-pme`" wires "the MCP server *and* ambient capture in two lines." Source: README (accessed 2026-08-27), cross-checked against https://pypi.org/project/iai-pme/.

**The pitch was rewritten around token economics and the "cyber brain" frame.** The GitHub description now reads: "A cyber brain for your AI. It never forgets a detail, remembers exactly what you said, and learns how you work over time. Free, local, works with Cursor, Claude Code, Codex, OpenClaw, Hermes and more. MIT." The README headline: "Give your coding agent a brain that remembers exactly what you said — forever, on your machine," with the lead claim "Pays for itself in tokens: an injected memory pack costs ≈88% less than the agent search it displaces." The 88% figure is the vendor's own, "measured on the author's own store over three recent weeks," so treat it like the other self-reported benchmarks. A Chinese README (README_zh-CN.md) was added. Sources: GitHub API `description` field and README (both accessed 2026-08-27).

**Hooks grew from three to four, and the host list broadened.** `iai-mcp capture-hooks install --target claude|codex|cursor|antigravity|hermes|openclaw|all`. The fourth hook is per-turn injection: "`iai-mcp-per-turn-recall.sh` — serves the foresight pack: memories the engine expects the *next* turn to need, injected before you ask." So auto-injection now happens per turn, where the June record describes session-start only. OpenClaw gets MCP tools without ambient capture ("OpenClaw has no shell hooks, so this registers the MCP server instead"). Source: README (accessed 2026-08-27).

**Retrieval grew a lexical lane, which retires one line of the June record.** The retrieval table now lists five paths, including: "Lexical | `memory_search` | BM25 over the record surface, fused with the semantic lane." The June record's "There is no BM25 anywhere in the README description" is therefore stale. The primary recall path is unchanged in kind: "Cue recall | `memory_recall` | Hybrid ranking — cosine + graph degree + recency, fused in one pass. No LLM on the hot path." Source: README (accessed 2026-08-27).

**Storage checks out as continuous.** Still Hippo ("The storage engine — encrypted records, the vector index, and the graph in one local store"), still "encrypted at rest with AES-256-GCM," still SQLite-founded underneath ("These sit on a thin layer of proven, permissive primitives — SQLite, the `candle` tensor library, NumPy, and the audited `cryptography` AES implementation"). Two details moved: the key path is now `~/.iai-mcp/.crypto.key` (the June record says `.key`), and the vector/graph kernels are their own Rust code instead of hnswlib. Episodic records are now explicitly append-only with contradiction edges ("a correction is a new record plus an edge, never an overwrite"). Source: README (accessed 2026-08-27).

**Camp verdict: Vector engine, holds.** The substrate is still local embeddings (bge-small-en-v1.5, 384-dim, now computed by their own Rust embedder) plus graph links plus recency inside an opaque encrypted store, with ambient capture, a daemon, and nightly consolidation. The BM25 lane is a fused secondary path, an addition to the vector core rather than a replacement of it. Nothing here moves iai toward plain files: delete iai and you still keep an encrypted blob and a key.

**Stale facts in the record below, corrected here and left as written there:** stars 805, not 325. Forks 94, not 44. Latest release v3.0.8 (2026-08-23), not v1.2.1. PyPI package `iai-pme` exists (the 404 finding described a real state that has since changed). Key file is `.crypto.key`, not `.key`. Four capture hooks, not three, and injection is per-turn as well as session-start. Hook targets go beyond Claude Code and Codex (Cursor, Antigravity, Hermes, OpenClaw). The Rust extension is no longer just "performance-critical kernels" on a Python/hnswlib base: since v2.0.0 the storage and graph engines themselves are their Rust code. The "no BM25" retrieval claim is retired. Consequence for the comparison page: the CompareTable retrieval and capture cells and the FieldVerdicts card need small wording updates (BM25 lane, broader hook targets); the storage cell, split, and wins all still hold.
...
- [GitHub API: repo metadata (stars 805, forks 94, new description, pushed_at 2026-08-23)](https://api.github.com/repos/CodeAbra/iai-personal-memory-engine) - accessed 2026-08-27
- [GitHub API: releases (v1.2.1 through v3.0.8, 40 tags total)](https://api.github.com/repos/CodeAbra/iai-personal-memory-engine/releases) - accessed 2026-08-27
- [v2.0.0 release notes (Rust storage + graph engine, desktop app, lexical + semantic search)](https://github.com/CodeAbra/iai-personal-memory-engine/releases/tag/v2.0.0) - accessed 2026-08-27
- [v3.0.0 release notes (thirteen languages, encrypted spool, entity anchors, key rotation)](https://github.com/CodeAbra/iai-personal-memory-engine/releases/tag/v3.0.0) - accessed 2026-08-27
- [README (raw, main) - PyPI install, Claude Code plugin, four hooks, five retrieval paths, 88% token claim](https://raw.githubusercontent.com/CodeAbra/iai-personal-memory-engine/main/README.md) - accessed 2026-08-27
- [PyPI project page iai-pme (package now exists)](https://pypi.org/project/iai-pme/) - accessed 2026-08-27
...
- Added 2026-08-27: the update section's facts are from the live GitHub API, the release notes for v2.0.0 and v3.0.0, the raw README, and a live PyPI check (`iai-pme` 3.0.8 confirmed present). The 88% token-saving figure and all benchmark numbers remain vendor self-reported and were not reproduced. The Claude Code plugin and the desktop app were read from the README, not installed or exercised here. Stars 805 is a point-in-time snapshot at 2026-08-27.
