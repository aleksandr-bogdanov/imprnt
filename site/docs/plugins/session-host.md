...
> **In one line.** A dedicated, user-started browser that holds your logged-in sessions and hands a fresh login token to other plugins over a localhost port, so they get reliable auth without reverse-engineering each site.

## What it's for

Other plugins (the kleinanzeigen watcher today, mail and channels later) need to act on a site you are logged into. Reading the token from your daily browser races that browser's own token rotation and fails. The session host is one place you log into every site you want the system to manage. It keeps each site's session warm so the site refreshes its own short-lived token, and automation reads a fresh one on demand.
