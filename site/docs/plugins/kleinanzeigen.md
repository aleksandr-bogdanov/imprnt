...
> **In one line.** It mirrors your Kleinanzeigen buyer messages, sorts each one with plain regex (zero model), drafts the easy replies, and sends you a phone-sized digest. You tweak a draft and press send.

## What it's for

A marketplace inbox is full of scammers and lowballers. That is exactly where you do not want a resident AI agent with account access, because a hostile message can talk an agent into acting. A regex cannot be social-engineered. This watcher reads the inbox with deterministic code, the model only ever drafts the handful of messages the rules cannot place, and the send button stays human.
...
This is a data and watcher plugin. Its files live in `plugins/kleinanzeigen/`:
...
`sync` reads the auth token straight from your logged-in browser on this Mac. Run the whole pipeline offline with `KLEINANZEIGEN_FIXTURES=<dir>`. Nothing sends on its own, and no command is a daemon. You schedule sync, rate, and notify if you want them periodic. The send step is always yours.
