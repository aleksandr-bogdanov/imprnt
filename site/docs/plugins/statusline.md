...
A four-row **panel** at the bottom of your imp session: the model, git state, context fill, cost, rate-limit windows, and the time. A long session loses track of where it stands, and the status line keeps the numbers in front of you: which model is running, how full the context window is, what you have spent, how close you are to a rate limit, and the clock.

## How it works

A **harness** plugin. It renders four aligned rows:
