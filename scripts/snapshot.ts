#!/usr/bin/env bun
// knowful snapshot <src> --dest <relpath> [--vault DIR]
//
...
// raw/ afterward and fans each source out into vault notes; `knowful check` then reconciles coverage.
//
...
//   knowful snapshot ~/.claude/PAI/USER/TELOS --dest pai/USER/TELOS
//   knowful snapshot ./tax2025.csv          --dest tax-2025
...
if (!src || !dest) {
  console.error("usage: knowful snapshot <src> --dest <relpath> [--vault DIR]");
  process.exit(1);
}
