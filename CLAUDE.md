...
No task management. No auto-injected context. No background loop. No self-grading. No MCP/vector/embeddings on the vault. No sensitivity machinery (it's private by being private). **No `out/` zone for deliverables** — a produced artifact (an article, an export) lives in your artifacts dir and a vault note points to it; the vault holds knowledge, not outputs. Every module is a self-contained dir you can `rm -rf`. "It belongs" is not a reason to add it.
