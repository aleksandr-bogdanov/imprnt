  ...
  return page({
    ...
    body: [
      ...
      rows.length === 0
        ...
        : table(["entry", "kind", "wanted", "seen", "pid", "peak bytes", "limit mb", ""], rows),
      elsewhere,
      "<h2>asked for</h2>",
      acts.length === 0
...
export function metricsPage(args: { rows: MetricsRow[]; notice?: string | null }): string {
  ...
  return page({
    ...
    here: "/metrics",
    notice: args.notice,
    body: body === "" ? '<p class="empty">nothing has been measured yet.</p>' : body,
  });
}
