// Test infrastructure: two people's trees, each carrying its own checkout of
// the household's shared zone.
//
// L7: "the boundary is the person", "a person is a registry entry", "one shared
// zone is mounted into every vault". The zone is a MOUNT: one shared remote
// checked out inside each person's own vault, at `<vault>/vault/<mount>`. So an
// agent reads the zone through the grant on its own tree and reaches no other
// person's copy of it, and the people are `p1` and `p2` because the repository
// is public.
//
...
/** The folder name every vault carries the shared zone under. */
...
export interface PlantedPerson {
  ...
  origin: string;
  /** This person's own checkout of the shared zone, inside their own vault. */
  zonePath: string;
  /** A file name only this person's checkout of the zone has. */
  zoneMarker: string;
}

export interface PlantedTrees {
  dir: string;
  /** The folder name every vault carries the zone under. */
  mount: string;
  people: PlantedPerson[];
  person(id: string): PlantedPerson;
  other(id: string): PlantedPerson;
  /**
   * A symlink inside this person's vault pointing at a path outside it, and the
   * link's own path back. `realpathSync` of the link resolves to the target, so
   * a box that resolves before it grants denies the link and its target
   * together.
   */
  plantZoneSymlink(personId: string, target: string): string;
}
...
export function plantTrees(base: string, ids: string[] = ["p1", "p2"]): PlantedTrees {
  ...
  for (const id of ids) {
    ...
    // The zone checkout, inside this person's own vault. Its marker carries its
    // own name, so a probe that reads the file prints a string only that
    // checkout could have produced.
    const zonePath = join(tree, "vault", MOUNT);
    const zoneMarker = `marker-zone-${id}-${token()}.txt`;
    mkdirSync(zonePath, { recursive: true });
    writeFileSync(join(zonePath, zoneMarker), `the zone checkout of ${id} holds ${zoneMarker}\n`, "utf8");
    people.push({ id, tree, marker, origin, zonePath, zoneMarker });
  }
  ...
  return {
    ...
    plantZoneSymlink(personId, target) {
      const link = join(person(personId).tree, "vault", `link-out-${token()}`);
