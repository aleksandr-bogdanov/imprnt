  ...
  async () => {
    ...
    try {
      ...
      const probe = ["/bin/sh", "-c", [
        `echo control-ran`,
        `echo -n user-bus:; busctl --address=unix:path=/run/user/${uid}/bus get-property ${version} 2>&1 | head -1`,
        `echo -n system-bus:; busctl --system get-property ${version} 2>&1 | head -1`,
      ].join("; ")];
