// MSG-10. A clock line is written into the chat log once, even when the door
// that wrote it died before its diary line landed.
//
...
// spawned session is fed from (L2), and the line's own id is what keeps a
// replay out of it: every canonical line carries an id and the log skips an id
// it already holds.
//
...
test(
  "a clock line the door wrote before it died is not written into the chat log a second time by the door that replaces it, and a clock line for a different message is still written",
  async () => {
