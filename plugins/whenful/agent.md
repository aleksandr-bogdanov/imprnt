...
Whenful is the user's task scheduler (whenful.com). This plugin lets a knowful note show the **live
...
- `plugins/whenful/proposed/` — staging for any note the plugin proposes into the vault (you approve
  it via `knowful ingest --apply`). Used sparingly — only to graduate a *summary*, never one note per
  task.
...
- `bun plugins/whenful/check.ts` — the plugin's own integrity check (mirror staleness + orphan links).
  The core finds it via `knowful check --all`.
